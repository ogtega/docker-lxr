...
our @EXPORT = qw(
	$cardinality
	$servertype  $scheme   $hostname  $port        $restricthost
	@schemealiases  @hostaliases      @portaliases @restrictaliases
...
CTX_V3_SQLITE
			...
			} elsif	(	4 == $context_created
						&&	5 == $context_version
					) {
				print <<CTX_V4_RESTRICT;
${VTyellow}WARNING:${VTnorm} context v4 does not implement host restrictions
         Context is automatically upgraded in no-restriction mode.
CTX_V4_RESTRICT
				$restricthost = undef;
				@restrictaliases = (undef) x @hostaliases;
				open(SOURCE, '<', $ctxtfile);
				while (<SOURCE>) {
					last if m/associated with (.*)/
				}
...
sub contextSave {
	...
	# next line new in version 5
		print DEST "\$restricthost = ", defined($restricthost) ? $restricthost : 'undef', ";\n";
	# host aliases added in version 3
		...
		print DEST join ("\n                 , ", @portaliases);
		print DEST "\n                 );\n";
	# next group new in version 5
		print DEST '@restrictaliases = qw( ';
		print DEST join ("\n                     , ", @restrictaliases);
		print DEST "\n                     );\n";
	# bzck to version 3
...
sub contextServer {
	...
	if ($verbose > 1) {
		print "where:\n";
		print "  - ! is an optional flag (see below),\n";
		print "  - scheme is either http or https,\n";
		print "  - host_name can be given as an IPv4 address such as 123.45.67.89,\n";
		print "              an IPv6 address like [fe80::1234:90ab]\n";
		print "              or a domain name like localhost or lxr.url.example,\n";
		print "  - port is 1 to 65535.\n";
		print "Scheme: or :port must be specified if you want to restrict\n";
		print "access to the tree through the designated scheme or port.\n";
		print "This restriction is requested with the ! prefix flag.\n";
	}
	...
	while (!defined($primaryhost)) {
		...
		if	(	'P' eq $treematch
			&&	(	$primaryhost =~ m%//\[[[:xdigit:]]+(::?[[:xdigit:]]+)+\]%
				||	$primaryhost =~ m%//\d{1,3}(\.\d{1,3}){3}%
				)
			) {
			print "${VTred}ERROR:${VTnorm} numeric IP forbidden in prefix mode!\n";
			$primaryhost = undef;
			next;
		}
		$primaryhost =~ m%^!?([^/]+)?//(\[[^\]]+\]|[^:]+)(?::(\d+))?/?%;
		...
		$hostname = $2;
		$port     = $3;
		$restricthost = '!' eq substr($primaryhost, 0, 1);
		$restricthost &&= defined($1) || defined($3);
...
# 		$port   = 443 if 'https:' eq $1 && !defined($3);
	...
	while	('' ne	($aliashost = get_user_choice
		...
		if	(	'P' eq $treematch
			&&	(	$aliashost =~ m%//\[[[:xdigit:]]+(::?[[:xdigit:]]+)+\]%
				||	$aliashost =~ m%//\d{1,3}(\.\d{1,3}){3}%
				)
			) {
			print "${VTred}ERROR:${VTnorm} numeric IP forbidden in prefix mode!\n";
			next;
...
# 		$aliasport   = 443 if 'https:' eq $1 && !defined($3);
		...
		push	(@restrictaliases
				, '!' eq substr($aliashost, 0, 1)
					&& (defined($1) || defined($3))
				);
	}
END_HOST:
